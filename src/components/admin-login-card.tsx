...
export function AdminLoginCard() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!/^\d{6}$/.test(otp)) {
      setError("E-postana gelen 6 haneli kodu gir.");
      return;
    }
    ...
    <form onSubmit={submit} className="space-y-5" aria-busy={busy}>
      {phase === "email" ? (
        <label className="form-field" htmlFor="admin-email">
          E-posta
          <input id="admin-email" value={email} onChange={(event) => setEmail(event.target.value)} className="form-control" type="email" autoComplete="email" placeholder="ornek@eposta.com" required />
        </label>
      ) : (
        <div className="rounded-[16px] border border-[var(--color-primary-light)] bg-[var(--color-primary-soft)] p-5">
          ...
          <label className="form-field mt-4" htmlFor="admin-otp">Doğrulama kodu
            <input id="admin-otp" value={otp} onChange={(event) => setOtp(event.target.value.replace(/\D/g, "").slice(0, 6))} className="form-control text-center font-mono text-2xl tracking-[.32em]" inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" maxLength={6} autoFocus required />
          </label>
